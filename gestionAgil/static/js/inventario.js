 ...
 * Versión actualizada para coincidir con el nuevo HTML.
 */
const InventoryApp = {
    ...
    elements: {
        ...
        itemsContainer: null,
        addItemForm: null,
        exitForm: null, // <--- Actualizado
        exitItemDropdown: null // <--- Actualizado
    },
    Modal: {
        elements: { container: null, title: null, message: null, confirmBtn: null, cancelBtn: null, closeBtn: null },
        confirmCallback: null,
        init() {
            const ids = { container: 'app-modal', title: 'modal-title', message: 'modal-message', confirmBtn: 'modal-confirm-btn', cancelBtn: 'modal-cancel-btn', closeBtn: 'modal-close-btn' };
    ...
    init() {
        ...
        // Asignamos los elementos del DOM con los nuevos IDs
        ...
        this.elements.exitForm = document.getElementById('exit-form'); // <--- Actualizado
        this.elements.exitItemDropdown = document.getElementById('exit-item'); // <--- Actualizado
    ...
    setupEventListeners() {
        ...
        this.elements.exitForm?.addEventListener('submit', (e) => this.handleFormSubmit(e, 'registerMovement')); // <--- Actualizado
    ...
    async handleFormSubmit(event, action) {
        ...
        try {
            ...
            if (action === 'addItem') {
                ...
                endpoint = '/movements/';
                // Añadimos el tipo de movimiento manualmente ya que no está en el formulario
                body.tipo_movimiento = 'salida'; 
    ...
    populateItemsDropdown(items) {
        const dropdown = this.elements.exitItemDropdown; // <--- Actualizado
        if (!dropdown) return;
